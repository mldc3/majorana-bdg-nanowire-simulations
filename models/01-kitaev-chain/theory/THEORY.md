...
$$
c_i\lvert0\rangle_i=0,\qquad c_i^\dagger\lvert0\rangle_i=\lvert1\rangle_i,\qquad c_i\lvert1\rangle_i=\lvert0\rangle_i,\qquad c_i^\dagger\lvert1\rangle_i=0. 
$$
...
$$
c_i\lvert n_1\ldots n_M\rangle=(-1)^{\sum_{m<i}n_m}n_i\lvert n_1\ldots 0_i\ldots n_M\rangle,
$$

$$
c_i^\dagger\lvert n_1\ldots n_M\rangle=(-1)^{\sum_{m<i}n_m}(1-n_i)\lvert n_1\ldots 1_i\ldots n_M\rangle. 
$$
...
$$
=\frac{1}{4}\left[2+2i\gamma_{2i-1}\gamma_{2i}\right] =\frac{1}{2}\left(1+i\gamma_{2i-1}\gamma_{2i}\right) \rightarrow i\gamma_{2i-1}\gamma_{2i}=2n_i-1. \tag{3.17}
$$
...
$$
\hat Pc_i\hat P^{-1}=e^{i\pi\hat N_F}c_ie^{-i\pi\hat N_F}=e^{-i\pi}c_i=-c_i,
$$

$$
\hat Pc_i^\dagger\hat P^{-1}=e^{+i\pi}c_i^\dagger=-c_i^\dagger. 
$$
...
$$
H=\sum_{ij}c_i^\dagger h_{ij}c_j+\frac{1}{2}\sum_{ij}\left(c_i^\dagger\Delta_{ij}c_j^\dagger+c_j\Delta_{ij}^*c_i\right).
$$
...
$$
\sum_{ij}c_i^\dagger\Delta_{ij}c_j^\dagger=\frac{1}{2}\sum_{ij}\left(c_i^\dagger\Delta_{ij}c_j^\dagger+c_j^\dagger\Delta_{ji}c_i^\dagger\right) =\frac{1}{2}\sum_{ij}c_i^\dagger(\Delta_{ij}-\Delta_{ji})c_j^\dagger. 
$$
...
$$
\frac{1}{2}\Psi^\dagger H_{\mathrm{BdG}}\Psi=\frac{1}{2}c^\dagger hc+\frac{1}{2}c^\dagger\Delta c^\dagger-\frac{1}{2}c\Delta^*c-\frac{1}{2}ch^*c^\dagger.
$$

The last normal term must be reordered. In indices,

$$
-\frac{1}{2}\sum_{ij}c_ih_{ij}^*c_j^\dagger=-\frac{1}{2}\sum_{ij}h_{ij}^*(\delta_{ij}-c_j^\dagger c_i)
$$

$$
=-\frac{1}{2}\operatorname{Tr}h^*+\frac{1}{2}\sum_{ij}c_j^\dagger h_{ij}^*c_i
$$

$$
=-\frac{1}{2}\operatorname{Tr}h+\frac{1}{2}c^\dagger hc,
$$
...
$$
H=\frac{1}{2}\Psi^\dagger H_{\mathrm{BdG}}\Psi+\frac{1}{2}\operatorname{Tr}h.
$$
...
$$
H_{\mathrm{BdG}}^\dagger=\begin{pmatrix}h^\dagger&(-\Delta^*)^\dagger\\\Delta^\dagger&(-h^*)^\dagger\end{pmatrix}=\begin{pmatrix}h&-\Delta^T\\\Delta^\dagger&-h^*\end{pmatrix}. 
$$

Since $\Delta^T=-\Delta$ and $\Delta^\dagger=(\Delta^*)^T=-\Delta^*$, one obtains $H_{\mathrm{BdG}}^\dagger=H_{\mathrm{BdG}}$.
...
$$
\{\eta_n,\eta_m^\dagger\}=\sum_j\left(u_{nj}^*u_{mj}+v_{nj}^*v_{mj}\right)=\delta_{nm},
$$

$$
\{\eta_n,\eta_m\}=\sum_j\left(u_{nj}^*v_{mj}^*+v_{nj}^*u_{mj}^*\right)=0.
$$
...
$$
H_{\mathrm{BdG}}\Phi_n=E_n\Phi_n,\qquad \Phi_n=\begin{pmatrix}u_n\\v_n\end{pmatrix}. 
$$
...
$$
\mathcal{C}=\tau_xK,\qquad \tau_x=\begin{pmatrix}0&I_N\\I_N&0\end{pmatrix},
$$

where $K$ denotes complex conjugation. Acting on a vector,

$$
\mathcal{C}\begin{pmatrix}u\\v\end{pmatrix}=\begin{pmatrix}v^*\\u^*\end{pmatrix}.
$$
...
$$
h=\begin{pmatrix}-\mu&-t&0&0\\-t&-\mu&-t&0\\0&-t&-\mu&-t\\0&0&-t&-\mu\end{pmatrix},\qquad \Delta=\begin{pmatrix}0&\Delta&0&0\\-\Delta&0&\Delta&0\\0&-\Delta&0&\Delta\\0&0&-\Delta&0\end{pmatrix}. 
$$
...
$$
\Psi_k=\begin{pmatrix}c_k\\c_{-k}^\dagger\end{pmatrix}.
$$

A matrix consistent with the real-space convention is

$$
H_{\mathrm{BdG}}(k)=\begin{pmatrix}\xi_k&2i\Delta\sin k\\-2i\Delta\sin k&-\xi_k\end{pmatrix}=\xi_k\tau_z-2\Delta\sin k\,\tau_y.
$$
...
$$
0=\det[H_{\mathrm{BdG}}(k)-EI_2] =\det\begin{pmatrix}\xi_k-E&2i\Delta\sin k\\-2i\Delta\sin k&-\xi_k-E\end{pmatrix} =(\xi_k-E)(-\xi_k-E)-(2i\Delta\sin k)(-2i\Delta\sin k) 
$$

$$
=E^2-\xi_k^2-4\lvert\Delta\rvert^2\sin^2k \rightarrow E_{\pm}(k)=\pm\sqrt{(-\mu-2t\cos k)^2+(2\lvert\Delta\rvert\sin k)^2}. 
$$
...
$$
\mathcal{M}=\operatorname{sgn}[\xi_0\xi_\pi]. 
$$

Using $\xi_0=-\mu-2t$ and $\xi_\pi=-\mu+2t$,

$$
\mathcal{M}=\operatorname{sgn}[(-\mu-2t)(-\mu+2t)]=\operatorname{sgn}(\mu^2-4t^2). 
$$

Therefore

$$
\mathcal{M}=\begin{cases}-1,&\lvert\mu\rvert<2\lvert t\rvert\quad\text{topological},\\+1,&\lvert\mu\rvert>2\lvert t\rvert\quad\text{trivial}.
\end{cases} 
$$
