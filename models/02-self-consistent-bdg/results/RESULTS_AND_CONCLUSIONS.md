...
1. Result set and interpretation protocol
2. Self-consistent order parameter
   1. Convergence of the gap iteration
   2. Spatial pairing profile in three Zeeman regimes
   3. Pairing profile versus interaction strength
3. Bulk spectral diagnostics
   1. Bulk bands in three regimes
   2. Minimum bulk gap versus Zeeman energy
4. Finite-wire spectral diagnostics
   1. Finite spectrum versus Zeeman energy
   2. Finite-wire map in the $(\mu,E_Z)$ plane
5. Local spectral weight
   1. LDOS in trivial, intermediate, and topological regimes
6. Wavefunction and Majorana diagnostics
   1. State closest to zero: trivial versus topological
   2. Electron–hole content of the near-zero state
   3. Decomposition into left and right Majorana components
7. Finite-size and interaction dependence
   1. Energy splitting versus wire length
   2. Interaction strength and edge localization
...
### 2.1 Convergence of the gap iteration

![Figure 2.1: Convergence history of the self-consistent BdG loop.](figures/fig_selfconsistent_convergence.png)

**Figure 2.1:** Convergence history of the self-consistent BdG loop.
...
### 2.2 Spatial pairing profile in three Zeeman regimes

![Figure 2.2: Magnitude of the converged pairing profile in trivial, intermediate, and topological regimes.](figures/fig_selfconsistent_gap_profile.png)

**Figure 2.2:** Magnitude of the converged pairing profile in trivial, intermediate, and topological regimes.
...
### 2.3 Pairing profile versus interaction strength

![Figure 2.3: Self-consistent pairing profile for three effective attractive interactions.](figures/fig_selfconsistent_gap_vs_interaction.png)

**Figure 2.3:** Self-consistent pairing profile for three effective attractive interactions.
...
### 3.1 Bulk bands in three regimes

![Figure 3.1: Homogeneous bulk bands obtained from the central self-consistent gap.](figures/fig_selfconsistent_bulk_bands.png)

**Figure 3.1:** Homogeneous bulk bands obtained from the central self-consistent gap.

#### What is calculated and why

The central portion of each finite-wire gap profile is averaged to define a representative homogeneous $\Delta_{\mathrm{bulk}}$. The Bloch Hamiltonian $H(k)$ is then evaluated across the Brillouin zone. Solid curves are the analytical dispersion and dashed black curves are the direct eigenvalues of the $4\times4$ matrix.
...
### 3.2 Minimum bulk gap versus Zeeman energy

![Figure 3.2: Minimum positive bulk excitation energy versus Zeeman energy.](figures/fig_selfconsistent_bulk_gap_vs_zeeman.png)

**Figure 3.2:** Minimum positive bulk excitation energy versus Zeeman energy.

#### What is calculated and why

At each $E_Z$, a self-consistent finite wire is solved, the central gap is extracted, and the homogeneous $H(k)$ is scanned to obtain $\min_k E_-(k)$. The resulting curve is the bulk excitation gap, not the energy of a finite-wire boundary state.
...
### 4.1 Finite spectrum versus Zeeman energy

![Figure 4.1: Subgap finite-wire BdG levels as the Zeeman energy is swept.](figures/fig_selfconsistent_spectrum_vs_zeeman.png)

**Figure 4.1:** Subgap finite-wire BdG levels as the Zeeman energy is swept.
...
### 4.2 Finite-wire map in the $(\mu,E_Z)$ plane

![Figure 4.2: Logarithm of the minimum absolute finite-wire energy over chemical potential and Zeeman energy.](figures/fig_selfconsistent_mu_zeeman_map.png)

**Figure 4.2:** Logarithm of the minimum absolute finite-wire energy over chemical potential and Zeeman energy.
...
### 5.1 LDOS in trivial, intermediate, and topological regimes

![Figure 5.1: Position- and energy-resolved local density of states.](figures/fig_selfconsistent_ldos_three_regimes.png)

**Figure 5.1:** Position- and energy-resolved local density of states.
...
### 6.1 State closest to zero: trivial versus topological

![Figure 6.1: Normalized density of the BdG state closest to zero in two regimes.](figures/fig_selfconsistent_mode_trivial_topological.png)

**Figure 6.1:** Normalized density of the BdG state closest to zero in two regimes.
...
### 6.2 Electron-hole content of the near-zero state

![Figure 6.2: Electron and hole densities of the state closest to zero.](figures/fig_selfconsistent_particle_hole.png)

**Figure 6.2:** Electron and hole densities of the state closest to zero.
...
### 6.3 Decomposition into left and right Majorana components

![Figure 6.3: Spatial densities of the two Majorana combinations formed from the near-zero pair.](figures/fig_selfconsistent_majoranas.png)

**Figure 6.3:** Spatial densities of the two Majorana combinations formed from the near-zero pair.
...
### 7.1 Energy splitting versus wire length

![Figure 7.1: Minimum absolute energy versus the number of sites on a logarithmic scale.](figures/fig_selfconsistent_splitting_vs_length.png)

**Figure 7.1:** Minimum absolute energy versus the number of sites on a logarithmic scale.
...
A logarithmic vertical scale is essential because the splitting spans many orders of magnitude. A monotonic straight line is not generally expected: the oscillatory phase $k_F L+\varphi$ produces nodes and revivals within the exponential envelope.
...
### 7.2 Interaction strength and edge localization

![Figure 7.2: Near-zero-state density for three attractive interaction strengths.](figures/fig_selfconsistent_interaction_localization.png)

**Figure 7.2:** Near-zero-state density for three attractive interaction strengths.
